...
contract('SignedMultiOracle', (accounts) => {
  ...
  it('can set result with signature by data source', async () => {
    ...
    await oracle.setResultWithSignature(id1, RESULT1, signature1, { from: signer1 })
    ...
    await oracle.setResultWithSignature(id2, RESULT2, signature2, { from: signer2 })
  ...
  it('cannot be set by a different signer', async () => {
    ...
    await shouldFail(oracle.setResultWithSignature(id1, RESULT1, signature, { from: signer1}))
  })
  ...
  it('should emit ResultSet event', async () => {
    ...
    const { logs } = await oracle.setResultWithSignature(id1, RESULT1, signature, { from: signer1 })
    await expectEvent.inLogs(
      logs,
      'ResultSet',
      {_id: id1, _result: RESULT1, _sender: signer1 }
    )
  })
})
